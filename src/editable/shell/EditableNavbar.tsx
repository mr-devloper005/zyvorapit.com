...
import { EditableReaderNavbar } from '@/editable/shell/EditableReaderNavbar'
...
const variantClasses = {
  'compact-bar': {
    shell: 'border-b border-[#d8e3df]/80 bg-[#f7fbf6]/90 text-[#10241f] shadow-[0_10px_40px_rgba(16,36,31,0.06)] backdrop-blur-xl',
    logo: 'rounded-[1.1rem] border border-[#cbdad5] bg-white shadow-[0_10px_28px_rgba(16,36,31,0.08)]',
    active: 'bg-[#10241f] text-[#effaf3]',
    idle: 'text-[#557069] hover:bg-[#e8f1ed] hover:text-[#10241f]',
    cta: 'rounded-full bg-[#dfff6d] text-[#10241f] hover:bg-[#cbed56]',
    mobile: 'border-t border-[#d8e3df]/80 bg-[#f7fbf6]/98',
  },
  'editorial-bar': {
    ...
    return (
      <header className={cn('sticky top-0 z-50 w-full', palette.shell)}>
        <nav className="mx-auto flex h-20 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
          ...
          <form action="/search" method="get" role="search" className="hidden min-w-0 flex-1 items-center justify-center lg:flex">
            <label className={cn('relative flex w-full max-w-xl items-center rounded-full px-4 py-3', palette.search)}>
              <span className="sr-only">Search</span>
              <Search className="pointer-events-none absolute left-4 h-4 w-4 opacity-70" />
              <input
                type="search"
                name="q"
                placeholder="Find businesses, spaces, and local services"
                autoComplete="off"
                className="h-5 min-w-0 flex-1 bg-transparent pl-7 pr-3 text-sm outline-none placeholder:text-current/60"
              />
              <span className="ml-auto hidden items-center gap-1 text-xs opacity-75 md:flex">
        ...
        {isMobileMenuOpen && (
          <div className={palette.mobile}>
            <div className="space-y-2 px-4 py-4">
              <form action="/search" method="get" role="search" className={cn('mb-3 flex gap-2 rounded-2xl p-2', palette.search)} onSubmit={() => setIsMobileMenuOpen(false)}>
                <label className="sr-only" htmlFor="directory-mobile-search">Search</label>
                <input
                  id="directory-mobile-search"
                  type="search"
                  name="q"
                  placeholder="Find businesses, spaces, and services"
                  autoComplete="off"
                  className="min-w-0 flex-1 rounded-xl bg-transparent px-2 text-sm outline-none placeholder:text-current/60"
                />
                <button type="submit" className={cn('inline-flex items-center justify-center rounded-xl px-3 text-sm font-semibold', palette.cta)}>
  ...
  return (
    <header className={cn('sticky top-0 z-50 w-full', style.shell)}>
      <nav className={cn('mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 sm:px-6 lg:px-8', isFloating ? 'h-24 pt-4' : 'h-20')}>
        ...
        <form action="/search" method="get" role="search" className="hidden min-w-0 flex-1 justify-center lg:flex">
          <label className={cn(
            'relative flex w-full max-w-md items-center rounded-full border px-4 py-2.5 transition-colors',
            isFloating
              ? 'border-white/15 bg-white/10 text-white shadow-[0_12px_36px_rgba(0,0,0,0.10)] backdrop-blur placeholder:text-white/60'
              : 'border-current/10 bg-white/70 text-current shadow-[0_10px_28px_rgba(15,23,42,0.05)] hover:bg-white/90'
          )}>
            <span className="sr-only">Search</span>
            <Search className="pointer-events-none absolute left-4 h-4 w-4 opacity-60" />
            <input
              type="search"
              name="q"
              placeholder="Search posts"
              autoComplete="off"
              className="h-6 w-full bg-transparent pl-7 pr-2 text-sm outline-none placeholder:text-current/50"
            />
          </label>
        </form>
        ...
        <div className="flex shrink-0 items-center gap-2 sm:gap-3">
          ...
          <Link href="/search" className="hidden rounded-full px-3 py-2 text-sm font-semibold text-current/70 transition-colors hover:bg-current/10 hover:text-current md:inline-flex lg:hidden">
            Search
          </Link>
          <Link href="/login" className="hidden rounded-full px-4 py-2 text-sm font-semibold text-current/75 transition-colors hover:bg-current/10 hover:text-current md:inline-flex">
            Login
          </Link>
          <Link href="/signup" className={cn('hidden px-5 py-2.5 text-sm font-bold shadow-[0_14px_32px_rgba(16,36,31,0.16)] md:inline-flex', style.cta)}>
            Sign up
          </Link>
      ...
      {isMobileMenuOpen && (
        <div className={style.mobile}>
          <div className="space-y-2 px-4 py-4">
            <form action="/search" method="get" role="search" className="mb-3 flex gap-2 rounded-2xl border border-border bg-card p-2" onSubmit={() => setIsMobileMenuOpen(false)}>
              <label className="sr-only" htmlFor="navbar-mobile-search">Search</label>
              <input
                ...
                placeholder="Search posts..."
                autoComplete="off"
                className="min-w-0 flex-1 rounded-xl bg-transparent px-2 text-sm outline-none placeholder:text-muted-foreground"
              />
              <button type="submit" className={cn('inline-flex items-center justify-center rounded-xl px-3 text-sm font-semibold', style.cta)}>
                <Search className="h-4 w-4" />
                <span className="sr-only">Submit search</span>
              </button>
            </form>
            <div className="mb-3 grid grid-cols-2 gap-2">
              <Link href="/login" onClick={() => setIsMobileMenuOpen(false)} className="rounded-2xl border border-border bg-card px-4 py-3 text-center text-sm font-semibold text-foreground">
                Login
              </Link>
              <Link href="/signup" onClick={() => setIsMobileMenuOpen(false)} className={cn('rounded-2xl px-4 py-3 text-center text-sm font-bold', style.cta)}>
                Sign up
              </Link>
            </div>
            {mobileNavigation.map((item) => {
